...
void HCD_filter_hw(T I_x[IMG_HEIGHT][IMG_WIDTH], T I_y[IMG_HEIGHT][IMG_WIDTH], T output_img[IMG_HEIGHT][IMG_WIDTH]) {
    ...
    HCD_filter_hw_label0:for (int y = 1; y < IMG_HEIGHT - 1; y++) {
        HCD_filter_hw_label1:for (int x = 1; x < IMG_WIDTH - 1; x++) {
            ...
            HCD_filter_hw_label2:for (int ky = -1; ky <= 1; ky++) {
            	HCD_filter_hw_label3:for (int kx = -1; kx <= 1; kx++) {
                    int yy = y + ky;
                    int xx = x + kx;
...
void wrapped_HCD_filter_hw(AXI_VAL in_stream[SIZE * 2], AXI_VAL out_stream[SIZE]) {
 ...
 // Lecture de I_x
 wrapped_HCD_filter_hw_label0:for (int y = 0; y < IMG_HEIGHT; y++) {
     wrapped_HCD_filter_hw_label1:for (int x = 0; x < IMG_WIDTH; x++) {
 ...
 wrapped_HCD_filter_hw_label2:for (int y = 0; y < IMG_HEIGHT; y++) {
     wrapped_HCD_filter_hw_label3:for (int x = 0; x < IMG_WIDTH; x++) {
 ...
 wrapped_HCD_filter_hw_label4:for (int y = 0; y < IMG_HEIGHT; y++) {
     wrapped_HCD_filter_hw_label5:for (int x = 0; x < IMG_WIDTH; x++) {
         int idx = y * IMG_WIDTH + x;
